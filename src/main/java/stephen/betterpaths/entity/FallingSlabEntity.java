...
public class FallingSlabEntity extends FallingBlockEntity {
    ...
    private FallingSlabEntity(World world, double x, double y, double z, BlockState block) {
    ...
    public static FallingSlabEntity spawnFromBlock(World world, BlockPos pos, BlockState state) {
        FallingSlabEntity fallingSlabEntity = new FallingSlabEntity(world, (double)pos.getX() + 0.5, pos.getY(), (double)pos.getZ() + 0.5, state.contains(Properties.WATERLOGGED) ? state.with(Properties.WATERLOGGED, false) : state);
        world.setBlockState(pos, state.getFluidState().getBlockState(), Block.NOTIFY_ALL);
        world.spawnEntity(fallingSlabEntity);
        return fallingSlabEntity;
    }
    @Override
                ...
                if (!blockState.isOf(Blocks.MOVING_PISTON)) {
                    if (!this.destroyedOnLanding) {
                        ...
                        if (bl3 && bl5) {
                            ...
                            if (this.getWorld().getBlockState(blockPos).isOf(this.block.getBlock()) && this.getWorld().getBlockState(blockPos).get(TYPE) == SlabType.BOTTOM){
